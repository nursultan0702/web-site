<template>
      <div class="col-sm-6 pt-5">
        ...
        <v-expansion-panel>
          <v-expansion-panel-content v-for="(item,i) in gosvells" :key="`${item.id}`">
        ...
        <div class="card mb-3" v-if="getAuth">
          <div class="card-body">
            <h5 class="card-title"><label>Введите тему</label>
              <input v-model="newGosvellsTitle" type="text" class="form-control"
            ...
            <textarea class="form-control" aria-label="With textarea" maxlength="10000"
                      v-model="newGosvellsBody"></textarea>
            <v-divider></v-divider>
            <button @click.prevent="saveGosvells" class="btn btn-primary float-right">Сохранить</button>
          </div>
        </div>
      </div>
</template>
...
<script>
  export default {
    data() {
      return {
        newGosvellsTitle: null,
        newGosvellsBody: null,
        docs: null,
        file: []
      }
    },
    methods: {
      saveGosvells() {
        ...
        this.$store.dispatch("addGosvells", {
          title: this.newGosvellsTitle,
          body: this.newGosvellsBody,
          docs: docs,
          postDate: date,
          file: this.file[0]
        });
    ...
    computed: {
      gosvells() {
        return this.$store.getters.getGosvells
      },
      getAuth(){
    ...
    beforeCreate(){
      this.$store.dispatch('loadGosvells');
    },
    name: "home",
    components: {},
  };
